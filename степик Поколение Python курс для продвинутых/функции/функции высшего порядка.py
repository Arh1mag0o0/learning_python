...
def division(number):
    return number % 5 == 2 and number // 100 > 0
...
numbers_out = filter(division, numbers)
numbers_out = map(sque, numbers_out)
...
def division(number):
    return number % 7 == 0 and (-100 < number < -9 or 9 < number < 100)
...
print(sum(map(squ, filter(division, numbers))))
